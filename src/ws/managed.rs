...
use crate::ws::{
    ...
    WsDisconnectCause, WsEndpointHandler, WsErrorAction, WsMessageAction, WsParseOutcome,
    WsReconnectStrategy, WsSessionMode, WsSubscriptionManager,
};

#[derive(Debug)]
pub enum ManagedWsCommand {
    Shutdown,
}
...
async fn replay_public_subscriptions<H, W>(
    handler: &mut H,
    writer: &mut W,
) -> Result<(), WebSocketError>
where
    H: WsEndpointHandler,
    W: futures_util::Sink<crate::ws::WsFrame, Error = WebSocketError> + Unpin,
{
    if handler.session_mode() != WsSessionMode::Public {
        return Ok(());
    }
    ...
    for subscription in subscriptions {
        ...
        writer.send(crate::ws::into_ws_message(payload)).await?;
    }
    Ok(())
}
...
pub async fn run_managed_ws_loop<T, H, R>(
    ...
    mut reconnect: R,
    status: Arc<AtomicU8>,
    mut cmd_rx: mpsc::UnboundedReceiver<ManagedWsCommand>,
) where
    T: WsTransport,
    H: WsEndpointHandler,
    R: WsReconnectStrategy,
{
    ...
    loop {
        ...
        if let Err(error) = replay_public_subscriptions(&mut handler, &mut writer).await {
            let cause = WsDisconnectCause::InternalError {
        ...
        let cause = loop {
            tokio::select! {
                command = cmd_rx.recv() => {
                    if matches!(command, Some(ManagedWsCommand::Shutdown) | None) {
...
async fn wait_reconnect_delay_or_shutdown(
    delay: Duration,
    cmd_rx: &mut mpsc::UnboundedReceiver<ManagedWsCommand>,
) -> bool {
    if delay.is_zero() {
        return true;
    }
